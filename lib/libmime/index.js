...
const encodeBase64 = (buf) => (typeof buf === 'string' ? Buffer.from(buf, 'utf-8') : buf).toString('base64');
...
var libmime = module.exports = {
    ...
    encodeWord: function (data, mimeWordEncoding, maxLength) {
        ...
        if (mimeWordEncoding === 'Q') {
            ...
            encodedStr = typeof data === 'string' ? data : encodeBase64(data);
            maxLength = maxLength ? Math.max(3, (maxLength - maxLength % 4) / 4 * 3) : 0;
        }


        if (maxLength && (mimeWordEncoding !== 'B' ? encodedStr : encodeBase64(data)).length > maxLength) {
            if (mimeWordEncoding === 'Q') {
    ...
    decodeWord: function (str) {
        ...
        if (encoding === 'B') {
            return libcharset.decode(decodeBase64(str), fromCharset);
        } else if (encoding === 'Q') {
